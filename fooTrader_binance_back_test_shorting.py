...
STATE = {"live":0,"long":0,"short":0,"games":0,"set_A":0,"set_B":0,"set_C":0,"simulation":1}
RSI_PERIOD =14
RSI_OVERBOUGHT =70
RSI_OVERSOLD = 30
...
def SHORT():
    ...
    if STATE["simulation"]==1:
        if STATE["live"]==0 and STATE["short"]==0:
            ...
            print(f'{CANDLES[i]["date"]} SHORTTTTT ENTRY: {avg_trade_entry_price}') 
...
def SHORT_CLOSE():
    ...
    if STATE["simulation"]==1:
        if STATE["live"]==1 and STATE["short"]==1:
            ...
            print(f'{CANDLES[i]["date"]} SHORT CLOSEDDD EXIT : {avg_trade_exit_price} profit {profit}')
...
for i in range(len(CANDLES)):
    ...
    # Sell
        if STATE["live"]==1 and STATE["long"]==1:
            ...
            if CANDLES[i]["close"] > avg_trade_entry_price + TAKE_PROFIT:
                SELL_MARKET()
    #short
        if STATE["live"]==0 and STATE["short"]==0:
            if CANDLES[i]["close"] > CANDLES[i]["EMA1"]: #above
                # redBar = CANDLES[i]["open"] > CANDLES[i]["close"] #redbar
                EMA_to_low_diff = CANDLES[i]["high"] - CANDLES[i]["EMA1"]
        ...
        if STATE["live"]==1 and STATE["short"]==1:
            ...
            if CANDLES[i]["close"] < avg_trade_entry_price + (-1*TAKE_PROFIT):
                SHORT_CLOSE()
...
print(f'running prof:{running_profit}')
