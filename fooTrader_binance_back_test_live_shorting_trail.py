...
symbol_lowercase = "dogeusdt"
interval_socket = "1m"

SOCKET = f'wss://stream.binance.com:9443/ws/{symbol_lowercase}@kline_{interval_socket}'
...
def BUY_MARKET():
    ...
    if STATE["live"]==0 and STATE["long"]==0:
        ...
        # else:
            ...
            email_Text(f'BUY ENTRY {avg_trade_entry_price}', f' diff {STRAT_DIFF} {vers}')
    ...
    if STATE["simulation"] ==0:
        if napper > 30:
            if STATE["live"]==0 and STATE["short"]==0:
                ...
                avg_trade_entry_price = trade_entry_prices[-1]
                trail_stop_price = avg_trade_entry_price + TRAIL_STOP_VALUE
                print(f'SHORT  {avg_trade_entry_price} @ {datetime.datetime.now()} trail: {trail_stop_price}')
                


                
                email_Text(f'SHORT ENTRY:{avg_trade_entry_price} trail: {trail_stop_price}', vers)
...
def on_message(ws,message):
    ...
    try:    
        ...
        if STATE["live"] == 1 and STATE["long"] == 1:
            ...
            #     SELL_MARKET()
        #stop Loss
            # if tick_price <= avg_trade_entry_price + (-1*STOP_LOSS):
            #     SELL_MARKET()
        ...
        #record Data!!!!
            ...
            #record Data!!!!
                ...
                print(f'date: {CANDLES[-1]["date"]}  close: {CANDLES[-1]["close"]} STRAT_DIFF: {STRAT_DIFF} EMA1: {CANDLES[-1]["EMA1"]}' )
        ...
        if barCount > 1:
            ...
            if CANDLES[-1]["EMA1"] > 0:
                ...
                if tick_price < CANDLES[-1]["EMA1"]: #below
                    ...
                    STRAT_DIFF = EMA_to_low_diff
                    
                        
                    if EMA_to_low_diff > diff_value and EMA_to_low_diff < 1:
            ...
            #shortClose
                if STATE["live"]==1 and STATE["short"]==1:
                    #tp
                    # if tick_price <= avg_trade_entry_price + (-1*TAKE_PROFIT):
                    #     SHORT_CLOSE()
                    ...
                    if STATE["live"] == 1 and STATE["long"] == 1:
                        trail_stop(TRAIL_STOP_VALUE)
                        print(f'Trailing...Tick {tick_price} vs trailstop {trail_stop(TRAIL_STOP_VALUE)}')
                        if tick_price >= trail_stop_price:
                            SELL_MARKET()
                ...
                if STATE["live"]==0 and STATE["short"]==0:
                    if tick_price > CANDLES[-1]["EMA1"]: #above
                        ...
                        if EMA_to_low_diff_short > diff_value + .001 and EMA_to_low_diff_short < 1:
