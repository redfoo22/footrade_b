    ...
    if STATE["simulation"] ==0:
        if napper > 30:
            if STATE["live"]==0 and STATE["short"]==0:
                ...
                # else:
                avg_trade_entry_price = trade_entry_prices[-1]
                print(f'SHORT   {CANDLES[-1]["close"]} @ {datetime.datetime.now()} STATE: {STATE["live"]} , {STATE["short"]} ')
...
def on_message(ws,message):
    try:    
        ...
        buy_position = STATE["live"] == 1 and STATE["long"] == 1
            ...
            #record Data!!!!
                ...
                if buy_position: 
                    if CANDLES[-1]["close"] >= avg_trade_entry_price + TAKE_PROFIT:
                        SELL_MARKET()
                if buy_position:
                    if CANDLES[-1]["close"] <= avg_trade_entry_price + (-1*STOP_LOSS):
                        SELL_MARKET()
        ...
        if barCount > 1:
            if buy_position != True:
                prev_redBar = CANDLES[-1]["open"] > CANDLES[-1]["close"]
                print(f' prev bar is red: {prev_redBar}')
                if prev_redBar:
                    prev_diff = CANDLES[-1]["open"] - CANDLES[-1]["close"]
                    print(f'prev_diff is {prev_diff}')
                    if prev_diff >= 0.005:
                        print("pannic bar")
                        panic_continuation_diff = CANDLES[-1]["close"] - tick_price
                        if panic_continuation_diff >= 0.0015:
                            BUY_MARKET()
