...
def on_message(ws,message):
    try:    
        ...
        # REAT TIME log 
        # if barCount > 1:
            # print(f'RSI {last_rsi} tick: {tick_price} EMA1: {CANDLES[-1]["EMA1"]}   live: {STATE["live"]} long {STATE["long"]} Trade Price: {avg_trade_entry_price} ')
        ...
        #         if last_rsi > 0 and last_rsi < 31:
        #             if STATE["live"] == 0 and STATE["long"] ==0:
        #                 BUY_MARKET()

                
                # buy to open in bar panic BARS
                if STATE["live"] == 0 and STATE["long"] == 0 and CANDLES[-1]["EMA1"] > 0:
                    if STATE["set_B"] == 1:
                        if CANDLES[-1]["open"] > CANDLES[-1]["close"]: #red
                            if CANDLES[-1]["close"] > CANDLES[-1]["EMA1"]: #above EMA1
                                print(" set_B red bar again")
                        if CANDLES[-1]["open"] > CANDLES[-1]["close"]: #green
                            BUY_MARKET()
                            print(f'Entering Trade at {avg_trade_entry_price}') 
                            STATE["set_A"] = 0
                            STATE["set_B"] = 0
                    ...
                    if STATE["set_A"] == 1:
                        if CANDLES[-1]["open"] > CANDLES[-1]["close"] and  CANDLES[-1]["close"] > CANDLES[-1]["EMA1"]: #red and ABOVE
                                STATE["set_B"] = 1
                                print(f'SETTING set_B red BAR = {STATE["set_B"]} at {datetime.datetime.now()}')
                        else:
                            STATE["set_A"] = 0
                            STATE["set_B"] = 0
                            print(f'RESETTING due to FAIL {datetime.datetime.now()}')
                    ...
                    if STATE["set_A"] == 1:
                        if CANDLES[-1]["open"] < CANDLES[-1]["close"]: #GREEN and ABOVE
                            if CANDLES[-1]["close"] > CANDLES[-1]["EMA1"]: #above EMA1
                                    STATE["set_A"] = 1
                                    print(f'SETTING set_A = {STATE["set_A"]} at {datetime.datetime.now()}')
