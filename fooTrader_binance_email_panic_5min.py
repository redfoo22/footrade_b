...
def BUY_MARKET():
    ...
    if bar_Napper >= 6:
        ...
        if STATE["live"]==0 and STATE["long"]==0:
            ...
            email_Text(foo_email,f'ENTRY {avg_trade_entry_price}', f'  {vers}') #Foo
            email_Text(alex_email,f'ENTRY {avg_trade_entry_price}', f' {vers}') #Alex
...
def SELL_MARKET():
    ...
    if STATE["live"]==1 and STATE["long"]==1:
        if napper > 10:
            ...
            email_Text(alex_email, f'Exit:{avg_trade_exit_price}', f'PROFIT:{profit} {vers} ')
            email_Text(foo_email, f'Exit:{avg_trade_exit_price}', f'PROFIT:{profit} {vers} ')
    ...
    try:    
        ...
        #record Data!!!!
                ...
                if STATE["live"] == 0 and STATE["long"] ==0:
                    if CANDLES[-1]["open"] > CANDLES[-1]["close"]: #red bar
                        ...
                        if diff  >= .017:
                            ...
                            email_Text(alex_email,f'Panic BAR {diff}',f'position: {STATE["live"]}/{STATE["long"]}')
                            email_Text(foo_email,f'Panic BAR {diff}',f'position: {STATE["live"]}/{STATE["long"]}')
                            BUY_MARKET()
            ...
            #TP
                if STATE["live"] == 1 and STATE["long"] ==1:
                    if CANDLES[-1]["close"] >= avg_trade_entry_price + TAKE_PROFIT:
                        email_Text(alex_email,f'TP CALLed',f'x')
                        email_Text(foo_email,f'TP CALLed',f'x')
                        SELL_MARKET()
            #STOP LOSS
