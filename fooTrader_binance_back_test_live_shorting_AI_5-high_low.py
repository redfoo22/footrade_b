...
def on_message(ws,message):
    ...
    try:    
        ...
        if barCount > 1:
            if tick_price < CANDLES[-1]["high_EMA"]:
                if STATE["trailing"] == 0:
                    TRAIL_STOP_VALUE = tick_price

                STATE["trailing"] = 1
                
        
        if STATE["trailing"]:
            print("TRAl VAL", TRAIL_STOP_VALUE)
            print("TRAILING ",STATE["trailing"])
            trail_baby(tick_price,CANDLES[-1]["high_EMA"]-CANDLES[-1]["low_EMA"],TRAIL_STOP_VALUE,"<")
            print(f'trail{trail_stop_price}')
            print(f'tickk{tick_price}')
            if STATE["live"]==0 and STATE["long"]==0:
