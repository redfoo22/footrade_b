...
STATE = {"live":0,"long":0,"short":0,"games":0,"set_A":0,"set_B":0,"set_C":0,"simulation":0,"history":0}
RSI_PERIOD =14
RSI_OVERBOUGHT =70
RSI_OVERSOLD = 30
...
def on_message(ws,message):
    try:    
        ...
        if STATE["live"] == 1 and STATE["long"] == 1: 
            if tick_price >= avg_trade_entry_price + diff_value:
                SELL_MARKET()
        #stop Loss
        ...
        #record Data!!!!
            if barCount > 1:
                ...
                print(F' {datetime.datetime.now()}high ema: {high_EMA}  EMA: {CANDLES[-1]["EMA1"]} low ema: {low_EMA}')
        ...
        if barCount > 1:
            ...
            if CANDLES[-1]["EMA1"] > 0:
                ...
                if tick_price < CANDLES[-1]["EMA1"]: #below
                    ...
                    diff_value = CANDLES[-1]["EMA1"]-CANDLES[-1]["low_EMA"]
                    print(F'buy diff {diff_value}, buy diff 667 {diff_value *.667}')
