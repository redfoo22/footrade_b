...
HISTORY_INTERVAL=Client.KLINE_INTERVAL_5MINUTE
TRADE_INTERVAL = "5m"
SHARES = 101
...
def trail_stop(value):
    ...
    if tick_price < avg_trade_entry_price:
        ...
        # print()
        trail_stop_price = trade_tick_low + value
...
def BUY_MARKET():
    ...
    if STATE["live"]==0 and STATE["long"]==0 and nap_bar >= 1:
        #state
...
for CAN in CANDLES:
    print(f' Date: {CAN["date"]} CLOSE: {CAN["close"]} EMA1: {CAN["EMA1"]} EMA2: {CAN["EMA2"]} ENTRY: {avg_trade_entry_price}')
print(len(CANDLES))
print(len(closes))
###################################### GET HISTORY BARS
        ...
        if barCount > 1:
            ...
            if STATE["live"] == 1 and STATE["short"] == 1:
                ...
                if tick_price > trail_stop(avg_trade_entry_price *.0065):
                    ...
                    print("TRAIL STOPPPPed OUT AT",trail_stop(avg_trade_entry_price *.0065))
                    SHORT_CLOSE()
